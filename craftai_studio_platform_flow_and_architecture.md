# CraftAI Studio — Production Architecture, System Flow & Technical Blueprint (MeiGen Benchmark)
...
**CraftAI Studio** combines a **high-fidelity AI Image & Video Creation Studio** with a **Curated Prompt Marketplace** (achieving full parity with MeiGen.ai), deployed across **Flutter Mobile** and **Next.js 15 Web Platform**.
...
| **2. AI Image Creation Studio** | • **Consistent Characters & Face Lock:** Save recurring character profiles with **3-angle facial reference photos** (Front, 45°, Side) using InstantID Apache 2.0; • Multi-reference image slots (`+ Add reference images`); • **AI Generation Engine Selector:** FLUX.1 Schnell (2 Cr), SDXL Turbo (2 Cr), SDXL Classic (3 Cr), InstantID Face-Lock (4 Cr); • Interactive Studio Bar: Batch Count (`- 1/4 +`), Seed Lock (`🔒`), Aspect Ratio (`Auto`, 1:1, 9:16, 16:9), Resolution (`2K / 4K`); • Dynamic button: `Generate ✨ {credits}` |
| **3. In-Prompt AI Actions & Expander** | • **AI Prompt Assistant Engine Selector:** Groq LPU Qwen 2.5 (`<0.8s`), Google Gemini 2.5 Flash (`Creative`), Claude 3.5 Sonnet (`Photoreal`), GPT-4o Mini (`Balanced`); • **Prompt Chat Copilot:** Multi-turn conversational delta compilation (`/prompt-engineering/chat-delta`) with turn-cap enforcement (max 5 free turns) and live engine switcher; • **`Enhance` (Magic Expander):** 1-tap expansion using selected AI Prompt Assistant; • **`✨ AI Edit`:** Instructed image modification with Subject Lock (`EDIT THE PROVIDED PHOTO — ABSOLUTE LOCK`); • Photo-to-Prompt vision scanner |
